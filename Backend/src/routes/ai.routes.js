...
router.post('/ai/next-question', authenticate, authorizeRoles('patient', 'doctor', 'admin'), async (request, response, next) => {
  ...
  try {
    ...
    result.nextQuestion = result.nextQuestion ? await translateQuestion(result.nextQuestion, normalizeLanguage(request.body.preferredLanguage)) : null;
    return success(response, result);
  } catch (error) {
...
router.post('/cases/:id/ai-question', authenticate, authorizeRoles('patient', 'doctor'), async (request, response, next) => {
  ...
  try {
    ...
    question.nextQuestion = question.nextQuestion ? await translateQuestion(question.nextQuestion, preferredLanguage) : null;
