...
function RoleSelection() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#fbfdfc] px-5 py-10 font-[DM_Sans,sans-serif] text-[#16302d]">
      <div className="w-full max-w-[720px] text-center">
        <div className="inline-flex items-center gap-3 text-left">
           <svg width="30" height="30" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div>
              <h2 className="m-0 text-2xl font-bold tracking-[-0.04em]">Swasthya</h2>
              <p className="m-0 mt-1 text-[7px] font-bold tracking-[0.14em] text-[#0d9488]">SMART INDIA HACKATHON</p>
            </div>
        </div>
        <h1 className="mt-8 text-[clamp(2rem,5vw,3rem)] font-bold tracking-[-0.05em]">How will you use Swasthya?</h1>
        <p className="mt-3 text-base text-[#607470]">Select your account type to access the pre-consultation portal.</p>
      </div>
      ...
      <div className="mt-10 grid w-full max-w-[760px] grid-cols-1 gap-5 sm:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#d9e8e3] bg-white p-7 text-center shadow-[0_12px_30px_rgb(22_60_56_/_6%)]">
          ...
          <h3 className="text-xl font-bold">I am a Patient</h3>
          <p className="my-3 min-h-14 text-sm leading-6 text-[#607470]">Explain your symptoms via voice/text, upload past laboratory reports, and generate structured sheets.</p>
          <button className="w-full rounded-md bg-[#0d9488] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#087b72]" onClick={() => navigate('/patient-login')}>
            Start Intake Guided System
          </button>
        </div>
      </div>

      <p className="mt-8 text-center text-xs text-[#8aa19b]">By continuing, you agree to our Terms of Service &amp; clinical workflow protocols.</p>
    </div>
  );
}
